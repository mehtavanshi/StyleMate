...
| --- | ----- | -------------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- | ------------ |
| C1  | [ ]   | `server/app/routers/wardrobe.py` | 1-64    | Imports nonexistent `WardrobeItem` model and undefined schemas. Router is dead code and not mounted in `main.py`. | Would immediately fail with `ImportError` if enabled. | **Critical** |
...
| #   | Check | File                                     | Line(s)  | Description                                                                                               | Impact                                                                         | Severity |
| --- | ----- | ---------------------------------------- | -------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ | -------- |
| H1  | [ ]   | `server/app/routers/calendar.py`         | 151-168  | `link_try_on_image()` auto-creates a `CalendarEntry` with hardcoded `user_id=1` when entry doesn't exist. | Calendar data can be attributed to the wrong user.                             | **High** |
| H2  | [ ]   | `server/app/services/weather_service.py` | 49       | `rule_for_temp()` fallback returns an integer instead of a rule dictionary.                               | Weather filtering can crash on invalid/extreme temperatures.                   | **High** |
| H3  | [ ]   | `server/app/pairing_engine.py`           | 860, 870 | Outfit explanation emits `"balanced silhouette"` twice from different scoring components.                 | Duplicate explanation fragments reduce explanation quality.                    | **High** |
...
| M3  | [ ]   | `server/app/style_match.py`                      | 848               | Uses `"olive green"` while color map only supports `"olive"`.                             | Color scoring and shopping queries fail for that recommendation. | **Medium** |
| M4  | [ ]   | `server/app/pairing_engine.py`                   | 191-195           | Fuzzy color matching depends on dictionary insertion order.                               | Ambiguous colors can resolve inconsistently.                     | **Medium** |
...
| M8  | [ ]   | `server/app/routers/tagging.py`                  | 482-496           | Color sanity check defaults confidence to `1.0` when confidence value is missing.         | Real confidence-writing failures become hidden.                  | **Medium** |
...
| #   | Check | File                             | Line(s) | Description                                                                               | Impact                                                              | Severity   |
| --- | ----- | -------------------------------- | ------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------- | ---------- |
| M12 | [ ]   | `app/app/(tabs)/index.tsx`       | 227-233 | Settings icon calls `router.push("/app/settings")` but correct route is `/settings`.      | Tapping Settings shows "no route found" error; navigation broken.   | **Medium** |
| M15 | [ ]   | `app/app/style-match.tsx` + `server/app/routers/style_match.py` | 73, 115-129 / 17-22 | Initial fetch uses default `limit=6` per category; load-more uses higher limit, returning better/diverse results. | Users see subpar static suggestions first — only getting good results after tapping "Load More". | **Medium** |
